...
impl std::fmt::Debug for ChronicleSigning {
...
impl WithSecret for ChronicleSigning {
        ...
        T: Send,
    {
        ...
        let secret = self
            ...
            .lock()
            .await
            .require_secret_by_ref(&secret_ref)
            .await?;
...
impl<T: ChronicleSigner + WithSecret + Send + Sync> BatcherKnownKeyNamesSigner for T {
    ...
    // is required by sawtooth for batcher signatures
    #[instrument(skip(self,data), level = "trace", name = "batcher_sign", fields(namespace = BATCHER_NAMESPACE, pk = BATCHER_PK))]
    async fn batcher_sign(&self, data: &[u8]) -> Result<Vec<u8>, SecretError> {
    ...
    #[instrument(skip(self,data,signature), level = "trace", name = "batcher_verify", fields(namespace = BATCHER_NAMESPACE, pk = BATCHER_PK))]
    async fn batcher_verify(&self, data: &[u8], signature: &[u8]) -> Result<bool, SecretError> {
        self.verify(BATCHER_NAMESPACE, BATCHER_PK, data, signature)
            .await
    }

    #[instrument(skip(self), level = "trace", name = "batcher_verifying", fields(namespace = BATCHER_NAMESPACE, pk = BATCHER_PK))]
    async fn batcher_verifying(&self) -> Result<VerifyingKey, SecretError> {
...
impl<T: ChronicleSigner + WithSecret + Send + Sync> ChronicleKnownKeyNamesSigner for T {
    #[instrument(skip(self,data), level = "trace", name = "chronicle_sign", fields(namespace = CHRONICLE_NAMESPACE, pk = CHRONICLE_PK))]
    async fn chronicle_sign(&self, data: &[u8]) -> Result<Vec<u8>, SecretError> {
        Ok(self
            .sign(CHRONICLE_NAMESPACE, CHRONICLE_PK, data)
            .await?
            .to_vec())
    }

    #[instrument(skip(self,data,signature), level = "trace", name = "chronicle_verify", fields(namespace = CHRONICLE_NAMESPACE, pk = CHRONICLE_PK))]
    async fn chronicle_verify(&self, data: &[u8], signature: &[u8]) -> Result<bool, SecretError> {
        self.verify(CHRONICLE_NAMESPACE, CHRONICLE_PK, data, signature)
            .await
    }

    #[instrument(skip(self), level = "trace", name = "chronicle_verifying", fields(namespace = CHRONICLE_NAMESPACE, pk = CHRONICLE_PK))]
    async fn chronicle_verifying(&self) -> Result<VerifyingKey, SecretError> {
...
impl<T: ChronicleSigner + WithSecret + Send + Sync> OpaKnownKeyNamesSigner for T {
    #[instrument(skip(self), level = "trace", name = "opa_sign", fields(namespace = OPA_NAMESPACE, pk = OPA_PK))]
    async fn opa_sign(&self, data: &[u8]) -> Result<Vec<u8>, SecretError> {
    ...
    #[instrument(skip(self,data,signature), level = "trace", name = "opa_verify", fields(namespace = OPA_NAMESPACE, pk = OPA_PK))]
    async fn opa_verify(&self, data: &[u8], signature: &[u8]) -> Result<bool, SecretError> {
        self.verify(OPA_NAMESPACE, OPA_PK, data, signature).await
    }

    #[instrument(skip(self), level = "trace", name = "opa_verifying", fields(namespace = OPA_NAMESPACE, pk = OPA_PK))]
    async fn opa_verifying(&self) -> Result<VerifyingKey, SecretError> {
        ...
        secrets
            .with_verifying_key(CHRONICLE_NAMESPACE, "chronicle-pk", |verifying_key| {
                assert_eq!(
                    ...
                    33,
                    "Verifying key should be 33 bytes"
                );
            })
            .await
            .unwrap();
