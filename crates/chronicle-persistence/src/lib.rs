...
type Generations = Vec<String>;
type Usages = Vec<String>;
type WasInformedBys = Vec<String>;
...
impl Store {
	...
	fn apply_entity(
		&self,
		connection: &mut PgConnection,
		Entity { namespace_id, external_id, domaintypeid, attributes, .. }: &Entity,
	) -> Result<(), StoreError> {
		use schema::entity::dsl;
		let (_, nsid) =
			self.namespace_by_external_id(connection, namespace_id.external_id_part())?;
	...
	#[instrument(skip_all)]
	pub fn populate_prov_model_for_agent(
		&self,
		agent: query::Agent,
	...
	#[instrument(skip_all)]
	pub fn populate_prov_model_for_activity(
		&self,
		...
		use diesel::prelude::*;
		use schema::{
			activity::dsl as activity_dsl, activity_attribute::dsl as activity_attr_dsl,
			agent::dsl as agent_dsl, association::dsl as association_dsl,
			entity::dsl as entity_dsl, generation::dsl as generation_dsl, usage::dsl as usage_dsl,
			wasinformedby::dsl as wasinformedby_dsl,
		};

		let attributes = activity_attr_dsl::activity_attribute
			.filter(activity_attr_dsl::activity_id.eq(&activity.id))
		...
		let (generations, usages, wasinformedbys, associations): (
			Generations,
			Usages,
			WasInformedBys,
			Associations,
		) = (
			generation_dsl::generation
				.filter(generation_dsl::activity_id.eq(activity.id))
				.order(generation_dsl::activity_id.asc())
				.inner_join(entity_dsl::entity)
				.select(entity_dsl::external_id)
				.load::<String>(connection)?,
			usage_dsl::usage
				.filter(usage_dsl::activity_id.eq(activity.id))
				.order(usage_dsl::activity_id.asc())
				.inner_join(entity_dsl::entity)
				.select(entity_dsl::external_id)
				.load::<String>(connection)?,
			wasinformedby_dsl::wasinformedby
				.filter(wasinformedby_dsl::activity_id.eq(activity.id))
				.inner_join(
					activity_dsl::activity
						.on(wasinformedby_dsl::informing_activity_id.eq(activity_dsl::id)),
				)
				.select(activity_dsl::external_id)
				.load::<String>(connection)?,
			association_dsl::association
				.filter(association_dsl::activity_id.eq(activity.id))
				.order(association_dsl::activity_id.asc())
				.inner_join(agent_dsl::agent)
				.select((agent_dsl::external_id, association_dsl::role))
		...
		for wasinformedby in wasinformedbys {
		...
		for (agent, role) in associations {
		...
		debug!(populate_entity = %model.summarize());

		Ok(())
	}

	#[instrument(skip_all)]
	pub fn populate_prov_model_for_entity(
		&self,
		entity: query::Entity,
		namespace_id: &NamespaceId,
		model: &mut ProvModel,
	...
	#[instrument(skip_all)]
	pub fn load_prov_model_for_namespace(
		&self,
		connection: &mut PgConnection,
		namespace: &NamespaceId,
		...
		for agent in agents {
			self.populate_prov_model_for_agent(agent, &namespaceid, &mut model, connection)?;
		}
		...
		for activity in activities {
			self.populate_prov_model_for_activity(activity, &namespaceid, &mut model, connection)?;
		}
		...
		for entity in entities {
			self.populate_prov_model_for_entity(entity, &namespaceid, &mut model, connection)?;
		}
	...
	pub fn prov_model_for_agent_id(
		...
		let mut model = ProvModel::default();
		self.populate_prov_model_for_agent(agent, &namespace, &mut model, connection)?;
		Ok(model)
	}
	...
	pub fn prov_model_for_activity_id(
		...
		let mut model = ProvModel::default();
		self.populate_prov_model_for_activity(activity, &namespace, &mut model, connection)?;
		Ok(model)
	}
	...
	pub fn prov_model_for_entity_id(
		...
		let mut model = ProvModel::default();
		self.populate_prov_model_for_entity(entity, &namespace, &mut model, connection)?;
		Ok(model)
	}
	...
	pub fn prov_model_for_usage(
			...
			.optional()?
		{
				...
				.optional()?
			{
				...
				self.populate_prov_model_for_entity(entity, &namespace, &mut model, connection)?;
				self.populate_prov_model_for_activity(
					activity, &namespace, &mut model, connection,
				)?;
			}
		}
		Ok(model)
