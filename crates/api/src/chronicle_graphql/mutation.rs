...
use common::{
	...
	identity::AuthId,
	prov::{ActivityId, AgentId, EntityId, operations::DerivationType, Role},
};

use crate::commands::{ActivityCommand, AgentCommand, ApiCommand, ApiResponse, EntityCommand};
use crate::dispatch::ApiDispatch;
...
pub async fn was_derived_from<'a>(
    ...
    derivation(ctx, namespace, generated_entity, used_entity, DerivationType::None).await
}
...
pub async fn was_revision_of<'a>(
    ...
    derivation(ctx, namespace, generated_entity, used_entity, DerivationType::Revision).await
}
...
pub async fn had_primary_source<'a>(
    ...
    derivation(ctx, namespace, generated_entity, used_entity, DerivationType::PrimarySource).await
}
...
pub async fn was_quoted_from<'a>(
    ...
    derivation(ctx, namespace, generated_entity, used_entity, DerivationType::Quotation).await
}
...
pub async fn used<'a>(
    ...
    let res = api
        .dispatch(
            ApiCommand::Activity(ActivityCommand::Use { id: entity, namespace, activity }),
            identity,
        )
        .await?;
...
pub async fn was_generated_by<'a>(
    ...
    let res = api
        .dispatch(
            ApiCommand::Activity(ActivityCommand::Generate { id: entity, namespace, activity }),
            identity,
        )
        .await?;
