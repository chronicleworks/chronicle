...
impl TransactionContext for SimulatedTransactionContext {
	...
	fn add_event(
		...
		self.tx.send(Some((Message_MessageType::CLIENT_EVENTS, stl_event))).unwrap();
		...
		match message_type {
			...
			Message_MessageType::CLIENT_BATCH_SUBMIT_REQUEST => {
				...
				for tx in batch.transactions {
					...
					self.handler.as_ref().apply(&req, &mut *self.context.lock().unwrap()).unwrap();
				}
			...
			Message_MessageType::CLIENT_BLOCK_GET_BY_NUM_REQUEST => {
				...
				response.set_block(Block { header: block_header_bytes, ..Default::default() });
			...
			Message_MessageType::CLIENT_BLOCK_LIST_REQUEST => {
				...
				let block_header_bytes = block_header.write_to_bytes().unwrap();
				response.set_blocks(
					vec![Block { header: block_header_bytes, ..Default::default() }].into(),
				);
...
impl EmbeddedChronicleTp {
	pub fn new_with_state(
		...
		let context = Arc::new(Mutex::new(SimulatedTransactionContext::new_with_state(tx, state)));
