...
use async_graphql::futures_util::select;
...
use chronicle_persistence::{MIGRATIONS, Store, StoreError};
use chronicle_signing::ChronicleSigning;
use common::attributes::Attributes;
use common::identity::AuthId;
use common::ledger::{Commit, SubmissionError, SubmissionStage};
use common::opa::PolicyAddress;
...
use common::prov::json_ld::ToJson;
use common::prov::operations::{ActivityExists, ActivityUses, ActsOnBehalfOf, AgentExists, ChronicleOperation, CreateNamespace, DerivationType, EndActivity, EntityDerive, EntityExists, SetAttributes, StartActivity, WasAssociatedWith, WasAttributedTo, WasGeneratedBy, WasInformedBy};
use protocol_substrate::SubxtClientError;
use protocol_substrate_chronicle::{ChronicleEvent, ChronicleTransaction};
use protocol_substrate_chronicle::protocol::{BlockId, FromBlock, LedgerReader, LedgerWriter};

use crate::{ApiError, ChronicleSigned};
use crate::commands::{ActivityCommand, AgentCommand, ApiCommand, ApiResponse, DepthChargeCommand, EntityCommand, ImportCommand, NamespaceCommand, QueryCommand};
use crate::dispatch::{ApiDispatch, ApiSendWithReply};
        ...
        + 'static,
{
    ...
    pub async fn new(
        ...
        Self::event_loop(ledger, signing, commit_rx, commit_notify_tx, store, reuse_reader, start_from_block);

        Self::depth_charge_loop(liveness_check_interval, &dispatch, system_namespace_uuid);

        Ok(dispatch)
    }

    fn depth_charge_loop(liveness_check_interval: Option<u64>, dispatch: &ApiDispatch, system_namespace_uuid: (&'static str, Uuid)) {
    ...
    fn event_loop(ledger: LEDGER, signing: ChronicleSigning, mut commit_rx: Receiver<ApiSendWithReply>, commit_notify_tx: Sender<SubmissionStage>, store: Store, reuse_reader: LEDGER, start_from_block: FromBlock) {
        tokio::task::spawn(async move {
    ...
    /// binding operation to tie the UUID from another instance to an external_id # Arguments
    ...
    ///
    /// We use our local store for the best guess at the activity, either by external_id or the last
    ...
    /// (ChronicleTransaction::Domaintype) if specified We use our local store for the best guess at
    ...
    async fn query(&self, query: QueryCommand) -> Result<ApiResponse, ApiError> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            Ok(ApiResponse::query_reply(api.store.load_prov_model_for_namespace(&mut connection, &id)?))
        })
            .await?
    }
    ...
    async fn use_agent_in_cli_context(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            connection.build_transaction().run(|connection| {
                api.store.apply_use_agent(connection, id.external_id_part(), &namespace)
            })?;
