...
use r2d2::Pool;
use tokio::sync::{broadcast::Sender, mpsc, mpsc::Receiver};
use tracing::{debug, error, info, info_span, instrument, trace, warn, Instrument};
use uuid::Uuid;

use chronicle_persistence::{Store, StoreError, MIGRATIONS};
...
use common::{
	...
	prov::{
		...
		ExternalIdPart, NamespaceId, ProvModel, Role, UuidPart, SYSTEM_ID, SYSTEM_UUID,
	},
};
...
use crate::{
	commands::{
		...
		ImportCommand, NamespaceCommand, QueryCommand,
	},
	dispatch::{ApiDispatch, ApiSendWithReply},
	ApiError, ChronicleSigned,
};
		...
		+ 'static,
{
	...
	fn depth_charge_loop(
		liveness_check_interval: Option<u64>,
		dispatch: &ApiDispatch,
		system_namespace_uuid: (&'static str, Uuid),
	) {
	...
	fn event_loop(
		ledger: LEDGER,
		signing: ChronicleSigning,
		mut commit_rx: Receiver<ApiSendWithReply>,
		commit_notify_tx: Sender<SubmissionStage>,
		store: Store,
		reuse_reader: LEDGER,
	...
	async fn query(&self, query: QueryCommand) -> Result<ApiResponse, ApiError> {
		...
		tokio::task::spawn_blocking(move || {
			...
			Ok(ApiResponse::query_reply(
				api.store.load_prov_model_for_namespace(&mut connection, &id)?,
			))
		})
		.await?
	}
