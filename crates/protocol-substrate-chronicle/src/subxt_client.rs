...
impl ChronicleEvent {
	#[tracing::instrument(level = "trace", skip(diff, identity), fields(diff = tracing::field::debug(&diff), identity = tracing::field::debug(&identity), correlation_id = tracing::field::debug(&correlation_id)))]
