...
/// definition, and writes the resulting schema files to the `domain-schema` directory. Collections
/// marked as not being "chronicle operations" are written to a file called
/// `exclude_collections.txt`.
///
/// # Arguments
///
...
fn generate_synth_collections(domain_file: &Path) -> Result<(), ChronicleSynthError> {
    ...
    println!("Generating schema for domain: {}.", generator.name.underline());
...
/// when generating the domain schema. See `generate` script in this repository for more
/// information.
...
mod tests {
    ...
    fn create_test_exclude_collections() -> Result<assert_fs::NamedTempFile, Box<dyn std::error::Error>> {
    ...
    #[test]
    fn test_append_to_exclude_list_skips_collections_already_on_list() -> Result<(), ChronicleSynthError> {
        let file = create_test_exclude_collections().unwrap();
