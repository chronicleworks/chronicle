	...
	pub fn executor_context(
		...
		match (policy_name_settings_entry, entrypoint_settings_entry) {
			...
			(Some(policy_name_settings_entry), Some(entrypoint_settings_entry)) => {
				...
				let policy_name =
					policy_name_settings_entry.get_entries().iter().next().ok_or_else(|| {
						ApplyError::InternalError("Invalid setting entry".to_string())
					})?;

				let policy_entrypoint =
					entrypoint_settings_entry.get_entries().iter().next().ok_or_else(|| {
						ApplyError::InternalError("Invalid setting entry".to_string())
					})?;
			...
			_ => Err(ApplyError::InternalError("Opa policy settings are invalid".to_string())),
		}
	}
}
