...
#[cfg(not(feature = "std"))]
use scale_info::{prelude::borrow::ToOwned, prelude::sync::Arc};
use serde::Serialize;
#[cfg(feature = "std")]
use std::{
...
impl fmt::Display for ProvSummary {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		write!(
            f,
            "{{ total_agents: {}, total_entities: {}, total_activities: {}, total_attributions: {}, total_derivations: {}, total_generations: {}, total_usages: {}, total_associations: {}, total_delegations: {} }}",
            self.total_agents,
            self.total_entities,
            self.total_activities,
            self.total_attributions,
...
impl ProvModel {
	...
	#[instrument(skip(self, tx), level = "trace", name = "apply_chronicle_operation", fields(
        op = ? tx, model = ? self
    ), ret(Debug))]
